...
//				Adding elements:
	...
	{
		//1) Запоминаем адрес удаляеиого элемента:
	...
	{
		//1) Доходим до предпоследнего элемента из списка:
		Element<T>* Temp = Head;
	...
	template<typename T>void ForwardList<T>::erase(T Data, int Index)
	{
...
//i-calur = r-value;
	...
	//Range-base for (for для контейнорв)
	for (int i : arr)
	{
		cout << i << tab;
	}
